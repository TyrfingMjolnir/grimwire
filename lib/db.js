...
module.exports = function() {
	...
	var usersDirPath = path.resolve('./users');
	if (!fs.existsSync(usersDirPath)) {
		winston.info('Creating users directory at '+usersDirPath);
		fs.mkdirSync(usersDirPath);
	}

	// Load data into memory from files
	winston.info('Reading users files from '+usersDirPath);
	var userFiles = fs.readdirSync(usersDirPath);
	...
	for (var i=0; i < userFiles.length; i++) {
		var filepath = path.join(usersDirPath, userFiles[i]);
	...
	function persistUser(id, cb) {
		...
		var filepath = path.join(usersDirPath, id);
