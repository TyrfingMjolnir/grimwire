...
module.exports = function(config, db) {
	...
	function createOnlineUser(userRecord, session) {
		return {
			id: session.user_id,
			num_user_streams: 0,
			max_user_streams: userRecord.max_user_streams || config.max_user_streams,
			num_guest_streams: 0,
			max_guest_streams: userRecord.max_guest_streams || 0,
	...
	server.post('/', function (req, res, next) {
		...
		db.countUsers(function(err, numUsers) {
			...
			if (numUsers >= 100) { // :TODO: make configurable
	...
	server.post('/', function (req, res, next) {
		...
		if (!body || !body.id || !body.password) {
			return res.send(422, { error: 'Must provide a username and password.' });
		}
		if (typeof body.id != 'string' || typeof body.password != 'string') {
			return res.send(422, { error: 'Username and password must be strings.' });
		}
		if (!/^[a-z0-9_]+$/i.test(body.id)) {
			return res.send(422, { error: 'Username can only contain letters, numbers, and underscores.' });
		}
		if (body.email && typeof body.email != 'string') {
			return res.send(422, { error: 'Email must be a string, if provided.' });
		}
		if (typeof body.max_guest_streams != 'undefined' && isNaN(body.max_guest_streams)) {
			return res.send(422, { error: 'Max guest streams must be a number, if provided.' });
		}
		if (body.id.length > 64) { return res.send(413, { error: 'Username must be 64 characters or less.' }); }
		if (body.password.length > 256) { return res.send(413, { error: 'Password must be 256 characters or less.' }); }
		if (body.email && body.email.length > 512) { return res.send(413, { error: 'Email must be 512 characters or less.' }); }
		if (body.avatar && body.avatar.length > 256) { return res.send(413, { error: 'Avatar must be 256 characters or less.' }); }
	...
	server.get('/:userId', function(req, res, next) {
		...
		db.getUser(req.params.userId, function(err, user) {
			...
			if (user.id == res.locals.session.user_id) {
				online_user = _online_users || {};
				item.num_user_streams = online_user.num_user_streams || 0;
				item.max_user_streams = online_user.max_user_streams || user.max_user_streams || config.max_user_streams;
				item.num_guest_streams = online_user.num_guest_streams || 0;
				item.max_guest_streams = online_user.max_guest_streams || user.max_guest_streams || 0;
			}
	...
	server.patch('/:userId', function (req, res, next) {
		...
		var updates = {};
		if (req.body.avatar) {
			if (typeof req.body.avatar != 'string') { return res.send(422, { error: 'Avatar must be a string.'}); }
			if (req.body.avatar.length > 256) { return res.send(413, { error: 'Avatar must be 256 characters or less' }); }
			updates.avatar = req.body.avatar;
		}
		if (req.body.email) {
			if (typeof req.body.email != 'string') { return res.send(422, { error: 'Email must be a string.'}); }
			if (req.body.email.length > 512) { return res.send(413, { error: 'Email must be 512 characters or less' }); }
			updates.email = req.body.email;
		}
		if (typeof req.body.max_guest_streams != 'undefined') {
			if (isNaN(req.body.max_guest_streams)) { return res.send(422, { error: 'Max guest streams must be a number.'}); }
			if (online_user && online_user.num_guest_streams > req.body.max_guest_streams) {
				return res.send(422, { error: 'Can\'t set the max number of guest streams to be less than the current number of guests.' });
			}
			updates.max_guest_streams = req.body.max_guest_streams;
		}
		...
		db.updateUser(req.params.userId, updates, function(err) {
			...
			db.updateSession(session.id, updates, function(err) {
				...
				if (online_user) {
					online_user.max_guest_streams = req.body.max_guest_streams;
				}
	...
	server.subscribe('/:userId/s/:appDomain/:streamId', function(req, res) {
		...
		// Store connection
		addStream(res, function(err, user) {
			if (err) {
				if (err.outOfStreams) {
	...
	function addStream(stream, cb) {
		...
		if (!user) {
			addUser(stream.locals.session, checkLimit);
		} else {
			checkLimit(null, user);
		}

		// Check whether the user has a stream available
		function checkLimit(err, user) {
			if (err) { return cb(err); }
			if (user.num_user_streams + user.num_guest_streams + 1 > user.max_user_streams) { return cb({ outOfStreams: true }); }
	...
	function removeUserStream(user, stream) {
