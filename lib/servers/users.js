...
module.exports = function(config, db) {
	...
	server.all('/', function (req, res, next) {
		...
		res.setHeader('Link', [
			...
			'</u/{user}/s/{app}/{stream}>; rel="grimwire.com/-links"'
		].join(', '));
		next();
	});
	server.all('/:userId', function (req, res, next) {
		...
		var links = [
			...
			'</u/'+userId+'/s/{app}/{stream}>; rel="grimwire.com/-links"'
		];
		if (user) {
			links = links.concat(user.links.map(serializeLinkObject));
			user.friends.forEach(function(friendId) {
				var friend = _online_users[friendId];
				if (friend) {
					links = links.concat(friend.links.map(serializeLinkObject));
				}
			});
		}
	...
	server.all('/:userId/s/:appDomain/:streamId', function(req, res, next) {
		...
		res.setHeader('Link', [
			'</>; rel="via service grimwire.com/-service"; title="Grimwire.net P2PW"',
			'</u/'+userId+'{?stream,nc}>; rel="up item grimwire.com/-p2pw/relay grimwire.com/-user"; id="'+userId+'"',
			'</u/'+userId+'/s/'+appDomain+'/'+streamId+'>; rel="self grimwire.com/-links"'
		].join(', '));
		next();
	});
		...
		function(req, res) {
			...
			for (var k in users) {
				...
				rows[user.id] = {
					id: user.id,
					online: !!onlineUser,
					links: (onlineUser) ? onlineUser.links : emptyArray,
					avatar: user.avatar,
	...
	server.get('/:userId', function(req, res, next) {
		...
		return res.json({ item: {
			...
			online: true,
			links: user.links,
			avatar: null, // :TODO: user.avatar,
			created_at: null // :TODO: user.created_at
		}});
	});
	...
	server.patch('/:userId', function (req, res, next) {
		...
		if (req.body.friends) {
			if (!Array.isArray(req.body.friends) || !req.body.friends.every(function(friend) { return typeof friend == 'string'; })) {
				return res.send(422, { error: '`friends` must be an array of strings.'});
			}
			updates.friends = req.body.friends;
		}
		// :TODO: email
		...
		db.updateUser(req.params.userId, updates, function(err) {
			...
			db.updateSession(session.id, updates, function(err) {
				...
				if (online_user) {
					if (updates.friends)
						online_user.friends = updates.friends;
				}
	...
	server.subscribe('/:userId', function(req, res) {
		...
		res.locals.app      = session.app || config.hostname;
	...
	// ---------------------
	server.notify('/:userId', function (req, res, next) {
		var session = res.locals.session;
		var app = session.app || config.hostname;
		...
		var srcParsed = relayIdRE.exec(body.src);
		if (!srcParsed) {
			return res.send(422, { error: '`src` is not a valid peer domain: must conform to /^(.+)@([^!]+)!([^:\\/]+)(?::([\\d]+))?$/i.' });
		}
		if (srcParsed[1] != session.user_id || srcParsed[3] != app.replace(':','.')) {
			return res.send(422, { error: '`src` must match the sending application (your session shows '+session.user_id+' and '+app+')' });
		}
	...
	server.get('/:userId/s/:appDomain/:streamId', function(req, res, next) {
		...
		// Fetch user & stream
		var relayId = createRelayId(req.params.userId, res.params.appDomain, res.params.streamId);
		...
		// Send response
		return res.json({ item: { links: links } });
	});
	...
	server.patch('/:userId/s/:appDomain/:streamId', function(req, res, next) {
		var session = res.locals.session;
		var relayId = createRelayId(req.params.userId, req.params.appDomain, req.params.streamId);
		...
		if (req.body.links) {
			...
			for (var i=0; i < links.length; i++) {
				...
				if (!link || typeof link != 'object' || !link.href) {
					res.writeHead(422, 'bad ent: link '+i+' did not parse into a link object');
					return res.end();
				}
	...
	function addStream(stream, cb) {
		...
		_online_relays[stream.locals.relayId] = stream;
		stream.on('close', onResStreamClosed);

		// Update user/app presence
		var user = _online_users[stream.locals.session.user_id];
		if (!user) {
			addUser(stream.locals.session, function(err, user) {
				if (err) { return cb(err); }
				addUserLinks(user, stream, [{ href: 'httpl://'+stream.locals.relayId, rel: 'service grimwire.com/-app' }]);
				cb(null, user);
			});
		} else {
			addUserLinks(user, stream, [{ href: 'httpl://'+stream.locals.relayId, rel: 'service grimwire.com/-app' }]);
	...
	function addUserLinks(user, stream, links) {
		...
		links.forEach(function(link) {
			link.user   = user.id;
			link.app    = stream.locals.app;
			link.stream = stream.locals.streamId;
		});
