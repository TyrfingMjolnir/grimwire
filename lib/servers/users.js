...
module.exports = function(config, db) {
	...
	function createRelayId(user, app, stream) {
		...
		return user+'@'+config.hostname+'!'+app+':'+(stream||'0');
	}
		...
		function(req, res) {
			...
			for (var k in users) {
				...
				rows[user.id] = {
					id: user.id,
					online: !!onlineUser,
					streams: (onlineUser) ? onlineUser.streams : emptyObj,
					avatar: user.avatar,
	...
	server.post('/:userId', function (req, res, next) {
		...
		if (!srcParsed) {
			return res.send(422, { error: '`src` is not a valid peer domain: must conform to /^(.+)@(.+)!(.+):([\\d]+)$/.' });
		}
		if (srcParsed[1] != session.user_id || srcParsed[3] != app) {
	...
	function addStream(res, cb) {
		var app = res.locals.session.app || config.hostname;
