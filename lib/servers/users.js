...
module.exports = function(config, db) {
		...
		function(req, res) {
			...
			for (var k in users) {
				...
				if (user.id == sessionUserId) {
					...
					rows[user.id].max_guest_streams = onlineUser.max_guest_streams || 0;
				}
			}
	...
	server.get('/:userId', function(req, res, next) {
		...
		db.getUser(req.params.userId, function(err, user) {
			...
			if (user.id == res.locals.session.user_id) {
				...
				item.max_user_streams = online_user.max_user_streams || 0;
				item.num_guest_streams = online_user.num_guest_streams || 0;
				item.max_guest_streams = online_user.max_guest_streams || 0;
			}
	...
	function addStream(stream, cb) {
		...
		function checkLimit(err, user) {
			...
			if (user.num_user_streams + 1 > user.max_user_streams) { return cb({ outOfStreams: true }); }
