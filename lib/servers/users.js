...
module.exports = function() {
	...
	server.getStatus = function() {
		return {
			num_streams: Object.keys(streams.online_streams).length,
			num_online_users: Object.keys(streams.online_users).length
		};
	};
	...
	server.patch('/:userId', function (req, res, next) {
		...
		if (typeof req.body.max_guest_streams != 'undefined') {
			...
			if (onlineUser && onlineUser.num_guest_streams > req.body.max_guest_streams) {
	...
	server.notify('/:userId/s/:appDomain/:streamId', function (req, res, next) {
		...
		if (srcParsed[1] != session.user_id || srcParsed[3] != session_app) {
