...
module.exports = function() {
	...
	server.all('/', function (req, res, next) {
		...
		var links = [
			...
			'</u{?online,links,link_bodies}>; rel="self collection gwr.io/relay/coll gwr.io/user/coll"; id="users"',
			'</u/{id}>; rel="item gwr.io/user/item"',
			'</u/{user}/s/{app}/{sid}{?nc}>; rel="item gwr.io/relay/item"'
		];
		if (req.query.links) {
	...
	server.all('/:userId', function (req, res, next) {
		...
		res.setHeader('Link', [
			...
			'</u{?online,links,link_bodies}>; rel="up collection gwr.io/relay/coll gwr.io/user/coll"; id="users"',
			'</u/'+userId+'>; rel="self item gwr.io/user/item"; id="'+userId+'"',
			'</u/'+userId+'/s/{app}/{sid}{?nc}>; rel="gwr.io/relay/item"'
		].join(', '));
		next();
	});
	server.all('/:userId/s/:appDomain/:sid', function(req, res, next) {
		...
		res.setHeader('Link', [
			...
			'</u/'+userId+'/s/'+appDomain+'/'+sid+'{?nc}>; rel="self item gwr.io/relay/item"'
		].join(', '));
		next();
	});
