...
module.exports = function(config, db) {
	...
	var hostnameAndPort = config.hostname+((config.port != 80 && config.port != 443) ? '.'+config.port : '');
	var relayIdRE = /^(.+)@(.+)!(.+):([\d]+)$/i;
	function createRelayId(user, app, stream) {
		...
		return user+'@'+hostnameAndPort+'!'+app+':'+(stream||'0');
	}
	...
	server.post('/:userId', function (req, res, next) {
		...
		if (srcParsed[1] != session.user_id || srcParsed[3] != session.app) {
			return res.send(422, { error: '`src` must match the sending application (your session shows '+session.user_id+' and '+session.app+')' });
		}
		...
		for (var i=0; i < body.dst.length; i++) {
			...
			if (!(body.dst[i] in _online_relays)) {
				console.log(body.dst[i],'not in',Object.keys(_online_relays))
				continue;
			}
