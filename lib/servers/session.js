...
module.exports = function() {
	...
	server.all('/', function (req, res, next) {
		...
		res.setHeader('Link', [
			...
			'</session>; rel="self service gwr.io/session"; type="user"; title="Sessions Service"',
			'</session/{app}>; rel="service gwr.io/session"; type="app"; title="3rd-party App Sessions Service"',
			'</session/{app}?guestof={hostuser}>; rel="service gwr.io/session"; type="guest"; title="Guest Sessions Service"'
		].join(', '));
		next();
	});
	server.all('/:app', function (req, res, next) {
		...
		res.setHeader('Link', [
			'</>; rel="up via service gwr.io/grimwire"; title="'+config.hostname+'"',
			'</session>; rel="service gwr.io/session"; type="user"; title="Sessions Service"',
			'</session/'+app+'>; rel="' + ((!isguest) ? 'self ' : '' ) + 'service gwr.io/session"; type="app"; title="3rd-party App Sessions Service"',
			'</session/'+app+'?guestof={hostuser}>; rel="' + ((isguest) ? 'self ' : '') + 'service gwr.io/session"; type="guest"; title="Guest Sessions Service"'
		].join(', '));
		next();
	});
