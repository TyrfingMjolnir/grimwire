...
streams.createOnlineGuest = function(guestUserId, hostUserId) {
	return {
		...
		guestof: hostUserId,
		num_user_streams: 0,
		max_user_streams: config.max_user_streams,
		streams: [], // an array of peerUris
...
streams.getOrCreateUser = function(userId, cb) {
	...
	db.getUser(userId, function(err, user) {
		...
		// Add to memory
		streams.online_users[userId] = streams.createOnlineUser(user);
...
streams.addStream = function(stream, cb) {
	...
	if (guestof) {
		...
		streams.getOrCreateUser(guestof, function(err, hostUser) {
			if (err) { return cb(err); }

			// Check stream limit
			if (hostUser.num_guest_streams + 1 > hostUser.max_guest_streams) { return cb({ outOfStreams: true }); }
			if (hostUser.num_user_streams + hostUser.num_guest_streams + 1 > hostUser.max_user_streams) { return cb({ outOfStreams: true }); }
		...
		// Get/create user
		streams.getOrCreateUser(userId, function(err, user) {
			if (err) { return cb(err); }
