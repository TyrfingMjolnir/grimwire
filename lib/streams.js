...
streams.addUserLinks = function(user, stream, links) {
	...
	links.forEach(function(link) {
		...
		Object.defineProperty(link, '__sid', { value: stream.locals.sid, enumerable: false });
	});
...
streams.removeUserLinks = function(user, stream) {
	user.links = user.links.filter(function(link) {
		return !(link.__sid == stream.locals.sid && link.__app == stream.locals.app);
	});
};
