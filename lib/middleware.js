...
module.exports.authenticate = function(config, db) {
	return function(req, res, next) {
		// Helper to send 401
		var authFail = function() {
			if (req.accepts('html')) {
				res.send(401, (config.settings.allow_signup) ? html.login_or_signup : html.login);
			} else {
				res.send(401);
			}
		};
		...
		// Fetch from DB
		db.getSession(sessionId, function(err, session) {
			if (err) {
				res.send(500);
