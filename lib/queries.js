...
module.exports = function(pgClient) {
	...
	db.createUser = function(data, cb) {
		var values = [data.id, data.email, data.password];
		pgClient.query('INSERT INTO users (id, email, password) VALUES ($1, $2, $3)', values, cb);
	};
	...
	db.deleteUserSessions = function(userId, cb) {
		pgClient.query('DELETE FROM sessions WHERE user_id = $1', [userId], cb);
	};
	return db;
};